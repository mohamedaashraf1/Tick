...
function render() {
  ...
  function renderTask(t) {
    ...
    const createdDate = dayjs(t.created).format('MMM D');
...
function initSortable() {
  ...
  sortable = Sortable.create(list, {
    ...
    onEnd: (evt) => {
      ...
      const itemId = list.children[evt.newIndex].dataset.id;
      const taskIdx = tasks.findIndex((t) => t.id === itemId);
      if (taskIdx === -1) return;
...
(function () {
  ...
  if (!gl) { console.warn('WebGL unavailable'); canvas.style.display='none'; return; }
